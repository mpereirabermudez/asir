...
function ip_recon() {
    read net_addr msk_addr < <(ip a | awk '/inet /{split($2, msk, "/"); split(msk[1], net, "."); if (net[1] != 127 && net[1] != 172) {print net[1]"."net[2]"."net[3]".0" " " msk[2]}}') # Dirección y máscara de red
    ...
    do
        ...
        then
            ...
            if nmap -p22 --open -T4 --min-rate 5000 -n -Pn "$ip_addr" | grep -q 'open' # Comprobamos si el puerto 22 (ssh) está abierto
            then
...
commands=("ip" "arp" "ping" "nmap")
for command in "${commands[@]}"
do
    if ! command -v "${command}" > /dev/null 2>&1 # Comprobamos si el comando está instalado
    then
        echo -e "${red}[!]${reset} ${yellow}${command}${reset} ${green}is not installed${reset}"
        num_commands=$((num_commands+1)) # Se incrementa el contador de comandos no instalados
    fi
done
if [[ $n_cmd -eq 0 ]] # Si el contador de comandos no instalados es distinto de 0, se aborta el script
then
    if  [[ $1 =~ ^([1-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-4])$ || $# -eq 0 ]] # Comprobamos si el argumento es un número entre 1 y 254
    then
        echo "                                                                                                                           ";
        echo "██╗██████╗     ██╗  ██╗ ██████╗ ███████╗████████╗    ██████╗ ██╗███████╗ ██████╗ ██████╗ ██╗   ██╗███████╗██████╗ ██╗   ██╗";
        echo "██║██╔══██╗    ██║  ██║██╔═══██╗██╔════╝╚══██╔══╝    ██╔══██╗██║██╔════╝██╔════╝██╔═══██╗██║   ██║██╔════╝██╔══██╗╚██╗ ██╔╝";
